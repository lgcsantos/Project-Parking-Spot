...
public class ParkingSpotController {
    ...
    public ResponseEntity<Object> saveParkinkSpot(@RequestBody @Valid ParkingSpotDto parkingSpotDto){
        ...
        if(ParkingSpotService.existsByApartmentAndBlock(parkingSpotDto.getApartment(), parkingSpotDto.getBlock())){
            return ResponseEntity.status(HttpStatus.CONFLICT).body("Conflict: Parking already registered for this apartment/block!");
        }
    ...
    public ResponseEntity<Object> getOneParkingSpot(@PathVariable(value = "id")UUID id){
        ...
        if (!parkingSpotModelOptional.isPresent()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Paking Spot Not Fould");
        }
    ...
    public ResponseEntity<Object> deleteParkingSpot(@PathVariable(value = "id") UUID id){
        Optional<ParkingSpotModel> parkingSpotModelOptional = parkingSpotService.findById(id);
        if (!parkingSpotModelOptional.isPresent()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Parking Spot Not Fould");
        }
